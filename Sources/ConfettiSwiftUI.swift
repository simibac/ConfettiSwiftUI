        ...
        for confetti in confettis {
            for color in colors {
                ...
                case .shape(_):
                    shapes.append(
                        AnyView(
                            confetti.view.foregroundColor(color).frame(
                                width: confettiSize, height: confettiSize,
                                alignment: .center)))
                case .image(_):
                    shapes.append(
                        AnyView(
                            confetti.view.frame(
                                maxWidth: confettiSize, maxHeight: confettiSize)
                        ))
                default:
                    shapes.append(
                        AnyView(
                            confetti.view.foregroundColor(color).font(
                                .system(size: confettiSize))))
                }
            }
        }

        _confettiConfig = StateObject(
            wrappedValue: ConfettiConfig(
                num: num,
                shapes: shapes,
                colors: colors,
...
struct ConfettiContainer: View {
    ...
    var body: some View {
        ...
        .onAppear {
            if firstAppear {
                DispatchQueue.main.asyncAfter(
                    deadline: .now() + confettiConfig.animationDuration
                ) {
...
struct ConfettiView: View {
    ...
    func getAnimationDuration() -> CGFloat {
        return 0.2 + confettiConfig.explosionAnimationDuration
            + getRandomExplosionTimeVariation()
    }

    func getAnimation() -> Animation {
        return Animation.timingCurve(
            0.1, 0.8, 0, 1, duration: getAnimationDuration())
    }

    func getDistance() -> CGFloat {
        return pow(CGFloat.random(in: 0.01...1), 2.0 / 7.0)
            * confettiConfig.radius
    }
    ...
    var body: some View {
        ConfettiAnimationView(
            shape: getShape(), color: getColor(), spinDirX: getSpinDirection(),
            spinDirZ: getSpinDirection()
        )
        ...
        .onAppear {
            withAnimation(getAnimation()) {
                ...
                if confettiConfig.openingAngle.degrees
            ...
            DispatchQueue.main.asyncAfter(
                deadline: .now() + getDelayBeforeRainAnimation()
            ) {
                withAnimation(
                    Animation.timingCurve(
                        0.12, 0, 0.39, 0,
                        duration: confettiConfig.rainAnimationDuration)
                ) {
                    location.y += confettiConfig.rainHeight
                    opacity =
                        confettiConfig.fadesOut ? 0 : confettiConfig.opacity
                }
            }
        }
    }
...
struct ConfettiAnimationView: View {
    ...
    var body: some View {
        shape
            ...
            .rotation3DEffect(
                .degrees(move ? 360 : 0), axis: (x: spinDirX, y: 0, z: 0)
            )
            .animation(
                Animation.linear(duration: xSpeed).repeatCount(
                    10, autoreverses: false), value: move
            )
            .rotation3DEffect(
            ...
            .animation(
                Animation.linear(duration: zSpeed).repeatForever(
                    autoreverses: false), value: move
            )
            .onAppear {
                if firstAppear {
                    move = true
...
class ConfettiConfig: ObservableObject {
    internal init(
        num: Int, shapes: [AnyView], colors: [Color], confettiSize: CGFloat,
        rainHeight: CGFloat,
        fadesOut: Bool, opacity: Double, openingAngle: Angle,
        closingAngle: Angle, radius: CGFloat,
